...
async function main() {
  ...
  if (senderBalance < requiredBalance) {
    console.error(
      `Insufficient balance to fund 10 wallets. Required: ${ethers.formatEther(
        requiredBalance
      )} ETH, Available: ${ethers.formatEther(senderBalance)} ETH`
    );
    return;
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // Fund the new wallet
    console.log(
      `Funding wallet ${i + 1} with ${ethers.formatEther(amountToSend)} ETH...`
    );
